...
Original file is located at
    https://colab.research.google.com/drive/1CnHpas7HrAzvSYuZAr4CThdH3hVfbRHk
...
# 指定資料集與 ZIP 檔案的本地路徑
data_dir = './plant_seedlings_classification/'  # 修改為你的資料夾路徑
zip_dir = './plant-seedlings-classification.zip'  # 修改為你的 ZIP 檔案路徑

# 若資料夾不存在，則解壓縮 ZIP 檔案
if not os.path.exists(data_dir):
    if os.path.exists(zip_dir):  # 檢查 ZIP 檔案是否存在
        with zipfile.ZipFile(zip_dir, 'r') as zip_ref:
            zip_ref.extractall(data_dir)
        print(f"資料已成功解壓縮到：{data_dir}")
    else:
        print(f"錯誤：找不到 ZIP 檔案：{zip_dir}")
else:
    print(f"資料夾已存在：{data_dir}")
...
def view_pred_result(preds, result_name, num_images_to_display=5):
    ...
    plt.tight_layout()
    plt.savefig(f'pred_result_{result_name}.png')
    plt.show()
...
view_pred_result(preds, "test_set")
...
def main():
    ...
    plt.savefig('Accuarcy_Curve.png')
    preds = predict(test_set, model)
    view_pred_result(preds,"main")
